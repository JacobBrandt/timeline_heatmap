...
function heatmap () {
    ...
    function heatmap(data, settings) {
      ...
      function createLegend() {
        legendHeight = 32;
        let legendMargin = {top: 0, right: 20, bottom: 50, left: margin.left};
      ...
      function filterOnLegend(minExtent, maxExtent) {
        graph.selectAll(".interval")
          .attr("fill", function(d) { return d.count >= minExtent && d.count <= maxExtent ? colorScale(d.count) : "none" })
          .attr("stroke", function(d) { return d.count >= minExtent && d.count <= maxExtent ? colorScale(d.count) : "none" })
      }

      function draw() {
        graph.selectAll(".swimlane").data(sources).enter()
          .append("g")
          .attr("class", "swimlane")
          ...
          .data(function(d) { return d.data; }).enter()
            .append("rect")
              ...
              .attr("width", function(d) { return xScale(d.time) - xScale(d.time-interval); })
